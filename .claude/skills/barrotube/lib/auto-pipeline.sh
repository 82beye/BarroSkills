...
# ─────────────────────────────────────────────────
# Stage C — Phase 8b: S8 QA 리포트 + SEO 보강
# ─────────────────────────────────────────────────
# produce-episode.js 는 S4~S9 중 S8(QA 리포트)을 만들지 않는다. 그런데 바로 다음
# Phase 9 의 guard_qa_pass 는 60_qa_report.md 가 없으면 "QA report 없음"으로 차단한다
# → cron 은 구조적으로 발행까지 도달할 수 없었다. 여기서 생성해 끊긴 고리를 잇는다.
log_stage "🧪 Phase 8b — S8 QA 리포트 + SEO 보강"

run_or_echo node scripts/automation/generate-qa-report.js \
  --episode "$EP_ID" --platform "$PLATFORM" \
  || fail_with_alert "Phase 8b QA" "generate-qa-report.js 실패"

# publish-youtube.js 는 SEO 필드가 비면 "SEO metadata missing"으로 업로드를 거부한다.
# 승인 토큰은 메타 해시에 결속되므로 반드시 Phase 10(승인)보다 먼저 돌려야 한다.
run_or_echo node scripts/automation/seo-enhance.js \
  --episode "$EP_DIR" --channel econ-daily \
  || fail_with_alert "Phase 8b SEO" "seo-enhance.js 실패"
...
if [ "$DRY_RUN" = "1" ]; then
  ...
  # 여기까지 왔다는 것은 publish_remains_human_only=false, 즉 무인 발행을 허용한
  # 상태다. 그런데도 승인 토큰을 사람이 만들어주길 기다리면 cron 은 영원히 발행에
  # 도달하지 못한다 — 토큰을 파이프라인이 발급하고, 실제 안전판은 바로 다음
  # Phase 11 의 텔레그램 거부 창구가 맡는다. approve-episode.js 자체가 QA PASS·
  # 썸네일 존재·정책 검사를 다시 확인하므로 게이트가 사라지는 것은 아니다.
  run_or_echo node scripts/automation/approve-episode.js \
    --episode "$EP_ID" --platform "$PLATFORM" \
    --by auto-pipeline --note "slot=${SLOT} 무인 승인 (거부 창구로 취소 가능)"
  if [ ! -s "$APPROVAL_PATH" ]; then
    notify_telegram "📋 <b>${EP_ID}</b> 자동 승인 실패 — 수동 승인 필요\n승인: <code>/approve ${EP_ID}</code>\n취소: <code>/cancel ${EP_ID}</code>"
    audit "auto_pipeline_awaiting_approval" "INFO" "slot=$SLOT ep=$EP_ID"
    echo "⏸  자동 승인 실패 — 수동 승인 대기: /approve ${EP_ID}"
    exit 0
  fi
  echo "✅ 무인 승인 토큰 발급: $APPROVAL_PATH"
...
# run-episode.js 는 --platform 을 받지 않는다(strict parseArgs → ERR_PARSE_ARGS_UNKNOWN_OPTION).
# 넘기면 업로드 직전에 즉사하므로 플랫폼은 EP 레이아웃에서 스스로 판별하게 둔다.
