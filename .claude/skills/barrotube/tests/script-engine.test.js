...
import { isExhausted } from '../scripts/automation/lib/text-engine.js';
...
test('the script step is not bound to a single provider', () => {
  ...
  const eng = readFileSync(join(ROOT, 'scripts', 'automation', 'lib', 'text-engine.js'), 'utf-8');
  assert.match(eng, /fallback = 'claude,codex,gemini'/,
...
test('the chain records which engine actually wrote the script', () => {
  ...
  const eng = readFileSync(join(ROOT, 'scripts', 'automation', 'lib', 'text-engine.js'), 'utf-8');
  assert.match(eng, /fallback from/, 'a fallback must be visible in the artifact');
...
test('codex runs read-only and hands the reply back through a temp file', () => {
  const src = readFileSync(join(ROOT, 'scripts', 'automation', 'lib', 'text-engine.js'), 'utf-8');
