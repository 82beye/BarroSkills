...
function buildUserPrompt(scenes, findings, retryFeedback) {
  ...
  // 예전 문구는 "수정 제안을 그대로 베끼지 말고 다듬어라" 였다. 그게 루프의 원인이었다:
  // 팩트체커가 출처로 검증해 준 문장을 모델이 일부러 바꿔 쓰면 그 표현이 다시 미검증이 되고,
  // 다음 팩트체크가 같은 씬을 새 사유로 잡는다. 3회 재작성이 그렇게 소진돼 사람을 불렀다
  // (2026-09-04 EP-0136, 09-05 EP-0138, 09-07 EP-0141 — 셋 다 손으로 제안을 거의 그대로
  //  적용하니 다음 판에 통과했다). 다듬는 범위를 연결·어조로 못 박는다.
  parts.push('팩트체커 수정 제안이 있으면 그 **사실 내용을 그대로 지켜라** — 수치, 폭·범위 표현,');
  parts.push('단정과 해석의 구분, 빼라고 지목된 요인은 바꾸지 마라. 그 문장은 출처로 검증된 것이다.');
  parts.push('다듬어도 되는 건 앞뒤 씬과의 연결과 어조까지다. 검증된 표현을 바꿔 쓰면 그 표현이');
  parts.push('다시 미검증이 되어 다음 팩트체크가 같은 씬을 또 잡는다.');
  // 2026-09-16 EP-2026-0157: 재작성이 설명을 통째로 지워 「올랐습니다. 배경은 나중에
  // 확인이 필요합니다」만 남겼다. 제목은 '급등한 이유'를 약속했는데 본문이 안 갚았다.
  // 원본 rev0 의 insight 는 「휴전 뒤 중동 재건과 무기 현대화 수요가 열릴 거란 기대 때문에
  // 매수세가 몰렸습니다」로, 정확히 시청자가 원하는 답이었다. 검증이 안 된다는 이유로 지워졌다.
  // 팩트체크가 지워야 할 것은 **틀린 것**이지 **검증 안 되는 것**이 아니다 —
  // 이 채널이 파는 건 해석이고, 해석은 원 출처로 증명되지 않는다.
  parts.push('');
  parts.push('[설명을 지우지 마라 — 표시만 바꿔라]');
  parts.push('"왜 그런가"를 말하는 문장이 검증되지 않았다고 **삭제하지 마라.** 삭제하면 이 채널은');
  parts.push('지수를 읊는 한 줄 뉴스가 된다. 대신 **단정에서 해석으로 표시를 바꿔라**:');
  parts.push('  ✗ 삭제  "정확한 상승 배경은 후속 보도로 다시 확인이 필요합니다"');
  parts.push('  ✗ 단정  "재건 수요 기대 때문에 매수세가 몰렸습니다"');
  parts.push('  ✓ 표시  "휴전 뒤 재건·무기 현대화 수요가 열릴 거란 기대가 작용했다는 해석이 나옵니다"');
  parts.push('  ✓ 표시  "시장은 종전보다 재건 수요를 먼저 본 것으로 보입니다"');
  parts.push('해석으로 표시하면 사실 주장이 아니므로 팩트체크 대상이 아니고, 시청자는 답을 얻는다.');
  parts.push('"확인이 필요합니다" "다시 봐야 합니다" 같은 **책임 회피 문장으로 대체하는 것은 금지**다 —');
  parts.push('시청자를 다른 곳으로 보내는 문장이라 60초를 쓴 이유가 사라진다.');
  parts.push('');
