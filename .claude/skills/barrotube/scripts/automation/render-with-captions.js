#!/usr/bin/env node
/**
 * render-with-captions.js — Grok 모션 렌더 위에 HyperFrames 로 자막을 얹어 최종본을 만든다.
 *
 ...
 *
 * produce-episode 의 S7 이 BT_CAPTION_ENGINE=hyperframes 일 때 render-direct.js 대신 이걸 부른다.
 * 산출물 경로가 같으므로 QA·승인·게시 단계는 바뀌지 않는다.
 *
 * Usage:
 *   node scripts/automation/render-with-captions.js --episode <dir> [--platform shorts]
 *     [--out <mp4>] [--keep-base]
 */
...
function main() {
  ...
  if (!values.episode) {
    console.error('Usage: render-with-captions.js --episode <dir> [--platform shorts] [--out sample.mp4]');
    process.exit(2);
  }
  ...
  const workDir = mkdtempSync(join(tmpdir(), 'bt-render-cap-'));
  try {
    ...
    const platform = values.platform || (meta.format === 'long-3min' ? 'long' : 'shorts');
    ...
    const outPath = resolve(values.out || join(baseDir, '55_render', 'video.mp4'));
    ...
    console.log(`\n✅ 렌더: ${outPath} (${totalSec.toFixed(1)}s)`);
  } finally {
