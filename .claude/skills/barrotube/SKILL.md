...
6. **S6 자산 생성** (비용 발생 — 운영자 명시 승인 필요):
   ...
   **S6c 씬 이미지·모션 클립 + S6d 인트로 — 기본: `barrotube-media-render` 스킬**
   ...
   - 모션 클립: `40_assets/videos/scene_NNN.mp4` — 신규 Shorts는 5/5 가 있어야 S6c 완료.
   ...
   - **인트로 카드 (ChatGPT): `45_intro.png`** — 에피소드 타이틀 대형 골드 타이포 +
     ...
     **타이틀은 상단 14% 아래에서 시작한다** — 그 위는 Shorts 플레이어의 상태바·
     뒤로가기·검색·⋮ 가 덮는다 (2026-08-27 EP-0118 실측 5.8%, 첫 줄이 가려졌다).
     하단 20%·우측 15%도 UI 구역이다. 값의 정본은 `lib/thumbnail-composer.js`의
     `SHORTS_SAFE`.
