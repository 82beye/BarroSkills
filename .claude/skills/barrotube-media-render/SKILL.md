...
2. **barrotube EP 모드 (S6c 씬 + S6d 인트로 소비자).** barrotube 스킬의 에피소드
   ...
   - **타이틀은 화면 상단 14% 아래에서 시작한다.** 인트로는 영상의 첫 화면이라 Shorts
     플레이어 UI 와 정면으로 겹친다 — 상단에는 상태바(시각·통신사·배터리)와 뒤로가기·
     검색·⋮ 아이콘이 얹힌다. 2026-08-27 EP-2026-0118 은 타이틀이 5.8% 에서 시작해
     첫 줄이 그 아이콘들 아래 깔렸고, EP-0117 도 8.6% 로 같았다. 안전한 회차
     (EP-0114 22.6% · EP-0116 24.4%)는 전부 20% 아래에서 시작했다.
     UI 가 차지하는 구역: **상단 14% · 하단 20%(채널·제목·설명·공유 바) ·
     우측 15%(좋아요·댓글·공유·리믹스)**. 글자와 핵심 오브젝트를 그 밖에 둔다.
     ⚠️ "최근 완료 EP 를 참고하라"는 아래 지시가 **틀린 배치도 복제한다** — 참고본의
     타이틀이 14% 위에서 시작하면 따라 하지 말고 안전구역을 우선한다.
